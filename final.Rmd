...
Through LGOCV, the minimal cross validation MSE for SVM is 0.3298 when cost is 0.5 and when L1 loss is used.
...
From resampling, we can see that random forest has the best prediction of life ladder values among the 10 methods, followed by KNN and GBM. 
The medium performance models includes boosted GAM, bagging tree, and bagging MARS.
Lastly, in predicting life ladder values, PCR, PLSR, elastic net, and SVM have similar MSEs, which are higher than other models.
From the density plot, we can see that the boosted GAM has the smallest variability.

After fitting the regression model with full dataset, we also splitted the data into training set and test set to fit regression and classification models.
